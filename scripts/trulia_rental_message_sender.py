...
def initialize_data_sheets(status_sheet, email_status_sheet, phone_status_sheet, df_input, LPI):
	df_status_list = ['handled']
	for i in range(1,(LPI*3) + 1):
	...
	df_email_list = []
	for i in range(1,(LPI*3) + 1): 
		df_email_list.append('address ' + str(i))
		df_email_list.append('total responses ' + str(i))
		df_email_list.append('response timestamp ' + str(i))
		df_email_list.append('total unavailable ' + str(i))
		df_email_list.append('unavailable ' + str(i))
	...
	df_status             = df_input[df_input.columns[0:8]] 
	...
	df_new                = (pd.DataFrame(columns=df_email_list)).fillna(' ')

	# create a new dataframe from the return sheet
	df_status_email       = df_input[df_input.columns[0:8]]
	# concatenate old values with the new columns
	df_status_email       = pd.concat([df_status_email, df_new], axis = 1).fillna(' ')
	df_status_email.to_csv(email_status_sheet,mode='wb',index=False)

	#empty dataframe with new columns for phone sheet
	df_new                = (pd.DataFrame(columns=df_phone_list)).fillna(' ')

	# create a new dataframe from the return sheet
	df_status_phone    = df_input[df_input.columns[0:8]]
	# concatenate old values with the new columns
	...
	return df_status, df_status_email,df_status_phone

def get_dataframes(names, status, email,phone, LPI): 
	df_input   = pd.read_csv(names) # create a dataframe with the return_sheet.
	if os.path.isfile(status) == False: # check if the status sheet exists
			print('Status Sheet Not Initialized\n Initializing ...')
			df_status, df_email,df_phone = initialize_data_sheets(status, email, phone, df_input, LPI)

	else: 
		# create a dataframe from saved status sheet csv
		df_status = pd.read_csv(status)
		df_email  = pd.read_csv(email) 
		df_phone  = pd.read_csv(phone)

	return df_input, df_status, df_email,df_phone

def get_partitions(df_rentals): 
	# cut the main rentals dataframe down by a criteria MAY NEED CHANGING IN THE FUTURE
	df_mod_rentals   = df_rentals[(df_rentals['Bedroom_max'] == '3') & (df_rentals['Bathroom_max'] == 2.0)].drop_duplicates(subset = 'Address')
	print(df_mod_rentals)
...
def send_message(name, email, phone_num, address,url):
...
while True:
	...
	if file_line is '':
		...
		while True:
			...
			df_input, df_status, df_status_email, df_status_phone = get_dataframes(names_sheet, status_sheet, email_status_sheet, phone_status_sheet, LPI)	# get all required dataframes 
			...
			print("HANDLING STATE " + str(handled_state) + ': ' + str(name))
			...
			df_race_cur_addr = df_race_cur_addr[~df_race_cur_addr['Address'].isin(race_prev_addr)]  # take the difference of the two above by addresses
			...
			time_stamp_col  = str('timestamp ' + str(handled_state+1))
	 		...
	 		while time_stamp == "NEED NEW ADDRESS":
				...
				#url            = "https://www.trulia.com/c/ca/los-angeles/4051-mclaughlin-4051-mclaughlin-ave-los-angeles-ca-90066--2077469244"
				time_stamp     = send_message(name, email, phone_num, address, url)  	# send out the inquiry and return the timestamp of when the inquiry was sent out
				while time_stamp == "RESTART DRIVER": 
					time_stamp  = send_message(name, email, phone_num, address, url)
			...
			df_status_email.set_value(row_index,address_string,address)

			df_status_phone.set_value(row_index, address_string, address)
